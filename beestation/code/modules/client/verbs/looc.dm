// LOOC ported from Monke, which was in turn ported from Bee, which was in turn ported from Citadel
...
/client/verb/looc(msg as text)
	...
	if(!holder)
		if(!CONFIG_GET(flag/looc_enabled))
			...
			return
		if(!GLOB.dooc_allowed && (mob.stat == DEAD) && SSticker.current_state < GAME_STATE_FINISHED)
		...
		if(findtext(msg, "byond://"))
			...
			return
		if(mob.stat && SSticker.current_state < GAME_STATE_FINISHED)
			to_chat(src, span_danger("You cannot salt in LOOC while unconscious or dead."))
			return
		if(isdead(mob) && SSticker.current_state < GAME_STATE_FINISHED)
